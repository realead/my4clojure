;;;;; Solution for 127. Love Triangle
...
(defn create-next-front
  [current-front dirs]
  (into #{} (for [x current-front
                  d dirs]
                 (vec (map + x d))
            )
  )
...
(defn front-ok?
    [current-front board]
    (every? #(contains? board %) current-front)
)

(defn max-triangle-area
    [pos dirs board]
    (loop [res 0
           cfront [pos]]
           (if (front-ok? cfront board)
               (recur (+ res (count cfront)) (create-next-front cfront dirs))
               res
           )
    )
)

(def DIRS [[[-1 1] [0 1] [1 1]];up
           [[0 1] [1 0]       ];up-right
           [[1 1] [1 0] [1 -1]];right
           [[1 0] [0 -1]      ];right-down
           [[1 -1] [0 -1] [-1 -1]];down
...
(defn get-set-bits
    ...
    (loop [res #{}
         ...
         (if (= z 0)
             res
             (recur (if (odd? z) (conj res [id index]) res) (inc index) (quot z 2))
         )
    )
)
...
(defn create-board
    ...
    (apply clojure.set/union
        (for [[id z] (map-indexed vector mine)] (get-set-bits id z))
    )
)

(defn find-max-triangle
    [mine]
    (let [board (create-board mine)
          area  (apply max (for [pos board
                                 dir DIRS] (max-triangle-area pos dir board)
                           )
                )]
        (if (< area 3);at least 3-rule
            nil
            area
        )
    )
