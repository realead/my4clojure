;;; Solution for 164. Language of DFA
...
(defn enumerate-language
   ([dfa] (enumerate-language [[(dfa :start) []]] (dfa :transitions) (dfa :accepts)))
   ([cur-queue trans accepted]
      (if (empty? cur-queue)
          '()
          (let [[cur-state cur-word] (first cur-queue)
                cur-queue (rest cur-queue)
                new-states-words (for [[ch state] (trans cur-state)] [state (conj cur-word ch)])]
                (lazy-seq (concat (map #(apply str (second %)) (filter #(contains? accepted (first %)) new-states-words))
                                  (enumerate-language (concat cur-queue new-states-words) trans accepted)
                          )
                )
          )
     )
...
(def __ enumerate-language)

(__ '{:states #{q0 q1 q2 q3}
              :alphabet #{a b c}
              :start q0
              :accepts #{q1 q2 q3}
              :transitions {q0 {a q1}
