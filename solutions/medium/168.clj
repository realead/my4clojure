...
(defn lazy-row
  [fun row start-col end-col]
    (if (= start-col end-col)
        '()
        (lazy-seq (cons (fun row start-col) (lazy-row fun row (inc start-col) end-col)))
    )
)


(defn lazy-sub-mat
    [fun start-row start-col end-row end-col]
    (if (= start-row end-row)
        '()
        (lazy-seq (cons (lazy-row fun start-row start-col end-col) (lazy-sub-mat fun (inc start-row) start-col end-row end-col)))
    )
)
...
(defn infinite-matrix
    ([fun] (lazy-sub-mat fun 0 0 -1 -1))
    ([fun start-row start-col] (lazy-sub-mat fun start-row start-col -1 -1))
    ([fun start-row start-col n-row n-col] (lazy-sub-mat fun start-row start-col (+ start-row n-row) (+ start-col n-col)))
)


(def __ infinite-matrix)
