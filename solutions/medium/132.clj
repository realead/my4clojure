;;;; Solution for 132. Insert between Items
...
(defn insert-inbetween
    [pred x col]
    (if (empty? col)
        col
        (flatten (reductions #(if (pred (last %1) %2) [x %2] [%2]) [(first col)] (rest col))); reduce wouldn't work because it isn't lazy
    )
)


(def __ insert-inbetween)
